...
def load_model():
  ...
  with open('final_model_architecture_v23.json', 'r') as f:
    model = model_from_json(f.read())

  # Load weights into the new model
  model.load_weights('final_model_weights_v23.h5')
  return model

model = load_model()
# model = Sequential()
# model.add(Dense(12,input_dim=feature_size,activation='relu'))
# model.add(Dense(24,input_dim=feature_size,activation='relu'))
...
def calc_reward(last_state,state):
  ...
  cur_to_target = Line(cur_self , target)
  last_to_cur = Line(last_self , cur_self)
  #movement_dist = float(cur_self.distance(last_self))
  return math.cos(last_to_cur.angle_between(last_to_target))
...
def replay(batch_size, epsilon):
  ...
  if cur_loss < min_loss and len(memory) >= 1000 :
    min_loss = cur_loss
    model.save_weights('final_model_weights_v24.h5')
    with open('final_model_architecture_v24.json', 'w') as f:
      f.write(model.to_json())

def main():
  print("MAIIIIIIIIIIIIIIN")
  ...
  for episode in itertools.count():
    ...
    # turn_ang = np.random.random() * 360.0
    hfo.act(DASH , 100 , 10)
    hfo.step()
    print("DASHHHHH")
